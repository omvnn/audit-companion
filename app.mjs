...
import { captureElementAnchor, restoreElementAnchor } from './scroll-position.mjs';
...
async function goAudit(id,message='',{anchor=null}={}){state.message=message;const [auditData,analyticsBundle]=await Promise.all([service.loadAudit(id),service.auditAnalyticsBundle(id)]);state.audit={...auditData,analyticsBundle};state.screen='audit';state.capa=null;state.analytics=null;state.importFlow=null;render();if(anchor)restoreElementAnchor(anchor);else window.scrollTo({top:0,behavior:'instant'});}
...
async function handleClick(btn){
  ...
  if(action==='save-response'){await withUi(async()=>{const card=cardFor(btn),anchor=captureElementAnchor(card);await saveCard(card);await goAudit(state.audit.audit.id,'Checklist item saved.',{anchor});});return;}
