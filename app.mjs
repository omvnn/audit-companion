...
const state={screen:'login',dashboard:null,audit:null,capa:null,analytics:null,message:'',inviteLink:'',busy:false};
...
async function goDashboard(message=''){state.message=message;state.inviteLink='';state.dashboard=await service.dashboard();state.screen='dashboard';state.audit=null;state.capa=null;state.analytics=null;render();}
async function goAudit(id,message=''){state.message=message;const [auditData,analyticsBundle]=await Promise.all([service.loadAudit(id),service.auditAnalyticsBundle(id)]);state.audit={...auditData,analyticsBundle};state.screen='audit';state.capa=null;state.analytics=null;render();window.scrollTo({top:0,behavior:'instant'});}
...
async function goCapa(filters={},message=''){const [{profile,labs},rows]=await Promise.all([managementContext(),service.capaRegister(filters)]);state.message=message;state.capa={profile,labs,rows,filters};state.screen='capa';state.audit=null;state.analytics=null;render();window.scrollTo({top:0,behavior:'instant'});}
async function goManagementAnalytics(filters={},message=''){const [{profile,labs},bundle]=await Promise.all([managementContext(),service.managementAnalytics(filters)]);state.message=message;state.analytics={profile,labs,filters,auditRows:bundle.audits,findingRows:bundle.findings,capaRows:bundle.capa};state.screen='analytics';state.audit=null;state.capa=null;render();window.scrollTo({top:0,behavior:'instant'});}
...
async function handleClick(btn){
  ...
  if(action==='logout'){api.clearSession();saveSession();state.screen='login';state.dashboard=null;state.audit=null;state.capa=null;state.analytics=null;state.message='Signed out.';render();return;}
  if(action==='edit-name'){qs('#profile-name-dialog')?.showModal();return;}
  if(action==='new-audit'){const dialog=qs('#audit-dialog');dialog?.showModal();updateScopeCount(qs('#audit-form'));return;}
